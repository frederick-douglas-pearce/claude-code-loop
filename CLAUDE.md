...
- **`tests/test_repo_consistency.py`** — **mechanical** checks on the markdown/JSON deliverable:
  ...
  `step N` cross-references — **65 sites, 69 numbers** — and `commands/init-loop.md`'s
  ...
  only 63: the other two are line-wrapped, which is exactly how they went unguarded until review
  ...
  **What a renumber still slips past** — the coverage claim is deliberately narrow, so read it
  ...
  mid-pipeline, renumber everything after it, and all 65 reference sites (69 numbers) point at the
