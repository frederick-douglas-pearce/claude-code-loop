...
- **F7's invocability claim — RETRACTED 2026-08-16 (#74), and the retraction is the finding.**
  ...
  **The method failure is the transferable part, and it is why this entry is rewritten rather than
  deleted.** F7 was recorded from **n=1**, generalized past what that observation established (locus:
  the refusal was about one argument, the claim was about the skill), never re-tested from the day it
  was recorded (2026-07-27) until #74 — and then given **explicit protection from re-testing** by the
  words "recorded so it is not re-litigated." That combination is why it survived a project otherwise
  aggressive about verifying claims: **the one note saying "do not re-check this" is the one that was
  wrong.**
